  ...
  >
    ...
    >
      <a-row :gutter="8">
        <a-col :span="24">
          <a-form-model-item label="Label name">
            <a-input
              v-model="fieldModel.label"
              @blur="labelChange"
              @input="watchLabelChange"
            />
          </a-form-model-item>
        </a-col>
      </a-row>
      <a-row :gutter="8">
        ...
        <a-col :span="6">
          <a-form-model-item label="Control type">
            <a-select v-model="fieldModel.fieldType">
              <a-select-option
                v-for="item in fieldTypeSelectList"
                :key="item.value"
                :value="item.value"
              >
                {{ item.value }}
        ...
        <a-col :span="6">
          <a-form-model-item label="Default value">
            ...
            <a-select
              v-if="showOptions"
              v-model="fieldModel.defaultValue"
              class="defaultSelect"
            >
              <a-select-option
                v-for="item in fieldModel.options"
                :key="item.value"
                :value="item.value"
              >
                {{ item.label }}
      ...
      <div class="validations_box">
        ...
        >
          <div
            v-for="(validation, validationIdx) in fieldModel.validations"
            :key="validation.index"
            class="validation_box draggable_validation pointer"
          >
            <a-row :gutter="8">
              <a-col :span="12">
                <a-form-model-item label="Name">
                  <a-radio-group
                    v-model="fieldModel.validations[validationIdx].name"
                  >
                    <a-radio
                      v-for="option in validationNameOptions"
                      :key="option.value"
                      :value="option.value"
                    >
                      {{ option.label }}
              ...
              <a-col :span="12">
                <a-form-model-item label="Error message">
                  <a-input
                    v-model="fieldModel.validations[validationIdx].errorMessage"
                  />
                </a-form-model-item>
              </a-col>
            </a-row>
            <a-row
              v-if="fieldModel.validations[validationIdx].name === 'pattern'"
              :gutter="8"
            >
              <a-col :span="24">
                <a-form-model-item label="Pattern">
                  <a-input
                    v-model="fieldModel.validations[validationIdx].pattern"
                  />
                </a-form-model-item>
              </a-col>
            </a-row>
            <a-row
              v-else-if="
                fieldModel.validations[validationIdx].name === 'validator'
              "
              :gutter="8"
            >
              <a-col :span="24">
                <a-form-model-item label="Customize validator">
                  <a-input
                    @input="
                      validationChange(fieldModel.validations[validationIdx])
                    "
                    row="1"
                    v-model="
                      fieldModel.validations[validationIdx].customizeValidator
                    "
                  />
                </a-form-model-item>
              </a-col>
      ...
      <a-row :gutter="8">
        <a-col :span="24">
          <a-form-model-item label="Calculated value">
            <a-input
              row="3"
              v-model="fieldModel.calculatedValue"
              @input="calculatedValueChange"
              @blur="calculatedValueBlue"
            />
          </a-form-model-item>
        </a-col>
      </a-row>
    ...
    >
      <div>
        ...
        >
          <a-row
            :gutter="8"
            v-for="(item, idx) in fieldModel.options"
            :key="item.index"
            class="draggable_option mb_3 pointer"
          >
            <a-col :span="12"
              ><a-input
                v-model="fieldModel.options[idx].label"
                @blur="labelChangeOption(item)"
            /></a-col>
            <a-col :span="12"
              ><a-input v-model="fieldModel.options[idx].value"
            /></a-col>
            <a-icon
              type="close"
              class="delete_option"
      ...
      <template slot="footer">
        <a-button type="primary" key="back" @click="toCloseOptionsModal"
          >Close</a-button
        >
      </template>
    </a-modal>
